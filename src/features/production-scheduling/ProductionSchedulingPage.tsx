...
import { selectAllProductionEvents, selectConfirmedQuantityByLotId, selectMaterialResourceEligibilities, selectOrganizationsByLotId, selectProductionConfirmations, selectProductionExecutions, selectProductionReadiness, selectProductionScheduling, selectScenarioDefinition, selectSessionClock, useScenarioStore, type ScheduleView, type Wf001ScenarioId } from '../../demo/scenario-engine/scenarioStore';
...
export function ProductionSchedulingPage() {
  ...
  const [simulationImpact, setSimulationImpact] = useState<SequenceMoveResult | null>(null);
  const [simulationRejection, setSimulationRejection] = useState<string | null>(null);
  ...
  /** Plano Atualizado (Section 1/21) — the SAME Unified Operational Timeline Acompanhamento and Aderência read, never a Plano-local reconstruction. */
  const timelineByLotId = operationalTimelineByRequirementId(computeOperationalTimeline(definition, executionsByLot, liveScenarioTime, events));
  ...
  const compromissoProduced = qualitySummary && qualitySummary.rows.some((row) => row.confirmations.length > 0) ? qualitySummary.produced : null;
  ...
  const runSimulation = (lotId: string, resourceId: FoundryResourceId, anchorLotId: string | null = null) => {
    const moving = definition.lots.find((lot) => lot.id === lotId);
    if (!moving) return;
    if (resourceId !== moving.scheduledResourceId) {
      // Cross-machine: Component -> Resource eligibility (Section 11) is validated here, in the feature layer — the engine itself stays pure scheduling math.
      ...
      if (!target?.eligible || target.status === 'BLOCKED') { setSimulationRejection(`${resourceId} não é Primary nem Reserve habilitada para este componente.`); return; }
    }
    const lockedLotIds = new Set(Object.entries(productionExecutions).filter(([, execution]) => execution.status !== 'NOT_STARTED').map(([id]) => id));
    const result = simulateSequenceMove(view.allScheduledLots, lotId, { kind: 'CROSS_RESOURCE', resourceId, anchorLotId }, lockedLotIds, 30);
    if (!result.feasible) {
      setSimulationRejection(result.reason === 'LOT_LOCKED' ? 'Este Lote já está em execução ou concluído — programação operacional bloqueada.' : result.reason === 'OVERLAP' || result.reason === 'ANCHOR_LOCKED_ORDER_VIOLATION' ? 'Esta posição entra em conflito com um Lote já em execução ou concluído nesta máquina.' : 'Lote não encontrado.');
      return;
    }
    setSelectedLot(moving);
    setSimulationImpact(result);
    setSimulationRejection(null);
  ...
  return (
    <OperationalWorkspace perspective="PLAN" lotId={selectedLot?.id ?? null} sidebarContent={<div className={styles.sidebarControls}>
      ...
      {(() => {
        ...
        const bufferCriticalTouched = Boolean(simulationImpact) && (simulationImpact!.movedLotId === BUFFER_CRITICAL_LOT_ID || simulationImpact!.affectedLotIds.includes(BUFFER_CRITICAL_LOT_ID));
        const bufferCriticalDelayed = bufferCriticalTouched && (simulationImpact!.originClosingDeltaMinutes > 0 || simulationImpact!.destinationClosingDeltaMinutes > 0);
        return bufferMaterial && bufferPosition ? <BufferDecisionSupport position={bufferPosition} material={bufferMaterial} criticalLot={definition.lots.find((lot) => lot.id === BUFFER_CRITICAL_LOT_ID)} simulationActive={simulationActive} hasConflict={bufferCriticalDelayed} /> : null;
      })()}
