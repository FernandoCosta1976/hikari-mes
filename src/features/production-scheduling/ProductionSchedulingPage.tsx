  ...
  return (
    <OperationalWorkspace perspective="PLAN" lotId={selectedLot?.id ?? null} sidebarContent={<div className={styles.sidebarControls}>
      ...
      <div className={styles.timelineLayout} data-detail-open={lotModalOpen ? 'true' : 'false'}>
        <div className={styles.timelineModeControls}><SimulationWorkspace active={simulationActive} selectedLot={selectedLot} allLots={view.allScheduledLots} readiness={selectedReadiness} impact={simulationImpact} rejection={simulationRejection} comparing={simulationComparing} alreadyReleased={selectedLot ? productionReleases[selectedLot.id]?.status === 'RELEASED' : false} locked={selectedLot ? (productionExecutions[selectedLot.id]?.status ?? 'NOT_STARTED') !== 'NOT_STARTED' : false} onActivate={() => { setSimulationActive(true); setLotModalOpen(false); }} onSimulate={(resourceId, anchorLotId) => selectedLot && runSimulation(selectedLot.id, resourceId, anchorLotId)} onUndo={() => { setSimulationImpact(null); setSimulationRejection(null); setSimulationComparing(false); }} onDiscard={() => { setSimulationActive(false); setSimulationImpact(null); setSimulationRejection(null); setSimulationComparing(false); }} onCompare={() => setSimulationComparing((value) => !value)} onAdopt={() => { if (simulationImpact) adoptOrganization({ lotId: simulationImpact.movedLotId, originalResourceId: simulationImpact.originResourceId, simulatedResourceId: simulationImpact.destinationResourceId }, 'Planejador da Fundição · demonstrativo'); setSimulationActive(false); setSimulationImpact(null); setSimulationRejection(null); setSimulationComparing(false); }} /></div>
