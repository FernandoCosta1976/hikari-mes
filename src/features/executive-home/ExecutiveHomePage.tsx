...
export function ExecutiveHomePage() {
  ...
  return <div className={styles.page}>
    <header className={styles.topbar}><a href="/demo/fundicao-dc" className={styles.brand} aria-label="Programa HIKARI — início"><span>H</span><b>HIKARI</b><small>MES</small></a><div className={styles.topbarActions}>{scenario ? <button type="button" onClick={resetScenario}>Reiniciar demonstração</button> : null}<a className={styles.workspaceLink} href="/demo/fundicao-dc/production-scheduling">Entrar na demonstração <span aria-hidden="true">→</span></a></div></header>
    <main>
      <section className={styles.hero} aria-labelledby="executive-title">
        <div className={styles.heroCopy}><span className={styles.eyebrow}>Primeira Onda · Fundação Operacional</span><h1 id="executive-title">PROGRAMA HIKARI</h1><h2>Da programação à visibilidade completa da eficiência operacional.</h2><p>Construímos progressivamente as capacidades para saber o que foi planejado, o que está acontecendo, por que houve desvios e qual foi o impacto na eficiência operacional.</p><div className={styles.heroActions}><a className={styles.primaryCta} href="/demo/fundicao-dc/production-scheduling"><span aria-hidden="true">▶</span> Iniciar demonstração</a><a href="#primeira-onda">Ver Primeira Onda ↓</a></div></div>
      ...
      <section id="primeira-onda" className={styles.section} aria-labelledby="first-wave-title"><header><span>01</span><div><h2 id="first-wave-title">Primeira Onda — Fundação Operacional</h2><p>Quatorze capacidades em quatro momentos, preservando a ordem que constrói a base para o OEE.</p></div></header><div className={styles.roadmap}>{moments.map((moment) => <section key={moment.id} className={styles.moment} data-moment={moment.id} aria-labelledby={`moment-${moment.id}`}><h3 id={`moment-${moment.id}`}>{moment.title}</h3><div>{moment.capabilities.map((capability) => { const content = <><span className={styles.capabilityNumber}>{capability.id}</span><strong>{capability.name}</strong><small className={styles.priority}>{capability.priority}</small><span className={styles.status} data-status={capability.status}>{statusLabel[capability.status]}</span></>; return capability.href ? <a key={capability.id} href={capability.href} aria-label={`${capability.id} ${capability.name}, ${statusLabel[capability.status]}`}>{content}</a> : <article key={capability.id} aria-label={`${capability.id} ${capability.name}, ${statusLabel[capability.status]}`}>{content}</article>; })}</div></section>)}</div><div className={styles.roadmapLegend}><span data-status="MATERIALIZED">Materializado</span><span data-status="NEXT">Próxima cena</span><span data-status="FUTURE">Futuro</span><strong>Foco da Primeira Onda: fundação operacional → OEE</strong></div></section>
      ...
      <section className={styles.finalCta}><div><h2>Pronto para a demonstração?</h2><p>Explore o Plano Hora-Hora por máquina e veja como o HIKARI conecta planejamento, preparação e decisões operacionais.</p></div><a className={styles.primaryCta} href="/demo/fundicao-dc/production-scheduling"><span aria-hidden="true">▶</span> Iniciar demonstração</a></section>
    </main>
