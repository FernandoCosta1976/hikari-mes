...
import { selectProductionReadiness, selectProductionScheduling, selectScenarioDefinition, useScenarioStore } from '../../demo/scenario-engine/scenarioStore';
...
export function ProductionReadinessPage() {
  ...
  const sidebarContent = <ReadinessQueue filter={filter} onFilter={(value) => { setFilter(value); setSelectedLotId(null); setDetailResourceId(null); window.history.replaceState(null, '', withBase('/demo/fundicao-dc/production-readiness')); }} />;
  if (!selectedLotId || !assessment || !lot || !material || !order || !schedule || !version) {
    ...
    const itemButton = (item: (typeof assessments)[number]) => { const candidate = scheduling.lots.find((entry) => entry.id === item.lotId)!; const candidateMaterial = scheduling.materials.find((entry) => entry.id === candidate.materialId)!; const reason = dominantReadinessCondition(item, candidate.scheduledResourceId); return <button key={item.lotId} onClick={() => { setSelectedLotId(item.lotId); window.history.replaceState(null, '', `?lotId=${item.lotId}`); }}><span>{statusIcons[item.status]}</span><strong>Lote {candidate.lotNumber}</strong><span>{candidateMaterial.name} · {candidate.quantity} peças</span><span className={styles.exceptionReason}>{reason?.label ?? item.summary}<small>{reason?.evidence}</small></span><Badge tone={statusTones[item.status]}>{statusLabels[item.status]}</Badge></button>; };
    return <OperationalWorkspace perspective="READINESS" sidebarContent={sidebarContent}><div className={styles.workspace}><nav className={styles.breadcrumb} aria-label="Origem"><a href={withBase('/demo/fundicao-dc/production-scheduling')}>Plano Hora-Hora</a><span>/</span><strong>Preparação</strong></nav><header className={styles.hero}><div><h1>Temos condições de produzir?</h1><p>Preparação do plano · cenário demonstrativo · exception-first · <strong>{formatDate(scheduling.schedules[0].businessDate)}</strong></p></div><Badge tone="attention">{aggregateCounts.BLOCKED + aggregateCounts.ATTENTION + aggregateCounts.UNKNOWN} Lots requerem análise</Badge></header><section className={styles.aggregateSummary} aria-label="Preparação do plano"><div><small>Preparação do plano</small><h2>{assessments.length} Lots no período</h2><p>Nenhum Lot foi selecionado silenciosamente. Resultado demonstrativo; regra de agregação não governada.</p></div><dl><div><dt>Impedimento</dt><dd>{aggregateCounts.BLOCKED}</dd></div><div><dt>Atenção</dt><dd>{aggregateCounts.ATTENTION}</dd></div><div><dt>Desconhecida</dt><dd>{aggregateCounts.UNKNOWN}</dd></div><div><dt>Atendidos</dt><dd>{aggregateCounts.READY}</dd></div></dl></section><section className={styles.priorityList} aria-labelledby="priority-title"><header><h2 id="priority-title">Fila operacional de exceções</h2><p>Impedimento → atenção → informação insuficiente. A ordem visual não representa prioridade operacional.</p></header>{exceptionLots.map(itemButton)}{readyLots.length ? <details className={styles.readyGroup}><summary>Prontos ({readyLots.length})</summary>{readyLots.map(itemButton)}</details> : null}{!priorityLots.length ? <p>Nenhum Lot corresponde ao filtro.</p> : null}</section></div></OperationalWorkspace>;
  }
  ...
  return <OperationalWorkspace perspective="READINESS" lotId={lot.id} sidebarContent={sidebarContent}>
    <div className={styles.workspace}>
      <nav className={styles.breadcrumb} aria-label="Origem"><button onClick={() => { setExpanded(useScenarioStore.getState().journeyContext?.sidebarExpanded ?? expanded); window.history.pushState(null, '', withBase(`/demo/fundicao-dc/production-scheduling?lotId=${lot.id}`)); window.dispatchEvent(new PopStateEvent('popstate')); }}>Plano Hora-Hora</button><span>/</span><span>Lote {lot.lotNumber}</span><span>/</span><strong>Preparação</strong></nav>
      <header className={styles.hero}><div><h1>Temos condições de produzir?</h1><p>Cenário demonstrativo · avaliação por exceção · {formatDate(schedule.businessDate)}</p></div><Badge tone={statusTones[assessment.status]}>{statusLabels[assessment.status]}</Badge></header>
      ...
      <section className={styles.summary} aria-labelledby="readiness-summary"><div><small>Preparação do Lote</small><h2 id="readiness-summary">{assessment.summary}</h2><p>Resultado provisório e explicável; não é liberação para execução.</p></div><dl><div><dt>OK</dt><dd>{counts.READY}</dd></div><div><dt>Atenção</dt><dd>{counts.ATTENTION}</dd></div><div><dt>Impedimento</dt><dd>{counts.BLOCKED}</dd></div><div><dt>Desconhecida</dt><dd>{counts.UNKNOWN}</dd></div></dl></section>
      <section className={styles.primaryCause} aria-label="Motivo ou bloqueio principal"><small>Motivo/Bloqueio principal</small><strong>{dominantCondition?.label ?? assessment.summary}</strong><span>{dominantCondition?.evidence ?? 'Nenhuma restrição dominante conhecida no cenário demonstrativo.'}</span><em>Resultado demonstrativo e explicável; não representa liberação.</em></section>
