...
export function ProductionReadinessPage() {
  ...
  return <OperationalWorkspace perspective="READINESS" lotId={lot.id} sidebarContent={sidebarContent}>
    <div className={styles.workspace}>
      ...
      <header className={styles.hero}><div><h1>Temos condições de produzir?</h1><p>Cenário demonstrativo · avaliação por exceção · {formatDate(schedule.businessDate)}</p></div><a href={withBase(`/demo/fundicao-dc/orders/${lot.id}`)}>Abrir Ordem →</a><Badge tone={statusTones[assessment.status]}>{statusLabels[assessment.status]}</Badge></header>
      ...
      <section className={styles.summary} aria-labelledby="readiness-summary"><div><small>Preparação do Lote</small><h2 id="readiness-summary">{assessment.summary}</h2><p>Resultado provisório e explicável; não é liberação para execução.</p></div><dl><div><dt>Atendidos</dt><dd>{counts.READY}</dd></div><div><dt>Atenção</dt><dd>{counts.ATTENTION}</dd></div><div><dt>Impedimento</dt><dd>{counts.BLOCKED}</dd></div><div><dt>Desconhecida</dt><dd>{counts.UNKNOWN}</dd></div></dl></section>
