...
export function ProductionReadinessPage() {
  ...
  const sidebarContent = <ReadinessQueue filter={filter} onFilter={(value) => { setFilter(value); setSelectedLotId(null); setDetailResourceId(null); window.history.replaceState(null, '', '/demo/fundicao-dc/production-readiness'); }} />;
  ...
  return <OperationalWorkspace perspective="READINESS" lotId={lot.id} sidebarContent={sidebarContent}>
    <div className={styles.workspace}>
      ...
      <section className={styles.handoff}><button onClick={() => { setExpanded(useScenarioStore.getState().journeyContext?.sidebarExpanded ?? expanded); window.history.pushState(null, '', `/demo/fundicao-dc/production-scheduling?lotId=${lot.id}`); window.dispatchEvent(new PopStateEvent('popstate')); }}>← Voltar ao Plano Hora-Hora</button><div><strong>{assessment.summary}</strong><small>Próxima experiência: organização nas máquinas. Nenhum Recurso será atribuído nesta etapa.</small></div><Button disabled>Organizar nas máquinas</Button></section>
    </div>
