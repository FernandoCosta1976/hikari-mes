...
function ContextDialog({ context, events, currentTime, nextLot, onClose }: { context: Context; events: readonly ProductionEvent[]; currentTime: string; nextLot?: Lot; onClose: () => void }) {
  ...
  return createPortal(<div className={styles.modalLayer}><button className={styles.backdrop} aria-label="Fechar contexto" onClick={onClose} /><section role="dialog" aria-modal="true" aria-labelledby="monitoring-context-title" className={styles.modal}>
    <header><div><small>{context.kind === 'LOT' ? 'ACOMPANHAMENTO DO LOT' : 'CONTEXTO DO RESOURCE'}</small><h2 id="monitoring-context-title">{context.kind === 'LOT' ? `Lote ${context.lot.lotNumber}` : context.resourceId}</h2></div><Button ref={close} aria-label="Fechar contexto de acompanhamento" onClick={onClose}>×</Button></header>
    <p>{context.kind === 'RESOURCE' ? `Lot atual: ${context.lot.lotNumber}` : `${context.execution.resourceId} · Schedule Version ${context.execution.scheduleVersionId}`}</p>
    <dl><div><dt>Estado atual</dt><dd>{stateLabel[context.execution.status]}</dd></div><div><dt>Progresso</dt><dd>{context.execution.producedQuantity} / {context.execution.plannedQuantity} · {Math.round(context.execution.producedQuantity / context.execution.plannedQuantity * 100)}%</dd></div><div><dt>Scheduled Start</dt><dd>{formatTime(context.lot.scheduledStart)}</dd></div><div><dt>Actual Start</dt><dd>{context.execution.actualStart ? formatTime(context.execution.actualStart) : '—'}</dd></div><div><dt>Actual Finish</dt><dd>{context.execution.actualFinish ? formatTime(context.execution.actualFinish) : '—'}</dd></div><div><dt>Evento ativo</dt><dd>{active ? `${eventLabel[active.eventType]} · ${eventDurationMinutes(active, currentTime)} min` : 'Nenhum'}</dd></div>{context.kind === 'RESOURCE' ? <div><dt>Próximo Lot</dt><dd>{nextLot ? `${nextLot.lotNumber} · ${formatTime(nextLot.scheduledStart)}` : 'Não conhecido'}</dd></div> : null}{mold ? <div><dt>Molde atual</dt><dd>{mold.code} · {Math.round(mold.lifeUsedRatio * 100)}% da vida útil{classifyMoldLife(mold.lifeUsedRatio) !== 'NORMAL' ? ` · ${moldStatusLabel[classifyMoldLife(mold.lifeUsedRatio)]}` : ''}</dd></div> : null}</dl>
    ...
    <small>Produced não significa estoque disponível, quantidade boa ou aceita.</small>
...
export function ProductionMonitoringPage() {
  ...
  return <OperationalWorkspace perspective="MONITORING" sidebarContent={<div className={styles.sidebar}><strong>Acompanhamento</strong><IconTip icon="◷" label="Business date" value="15/05" tip="Business date · 15/05/2025" /><IconTip icon="⏱" label="Current Time" value={formatTime(currentTime)} /><button onClick={reset}>Restaurar cenário</button><IconTip icon="ⓘ" label="Eventos demonstrativos" tip="Eventos demonstrativos · BUSINESS VALIDATION REQUIRED" /></div>}>
    <div className={styles.page}><header className={styles.hero}><div><span>Capability 06 · Core + Essencial</span><h1>O que está acontecendo na produção agora?</h1></div><aside><IconTip icon="⏱" label="Current Time" value={formatTime(currentTime)} tip="Horário de referência do cenário · passado, agora e futuro" className={styles.currentTimeTip} /></aside></header>
    <section className={styles.wip} aria-label="Resumo de WIP"><button onClick={() => openLot(executions.find((item) => item.status === 'NOT_STARTED')!)}><span>Aguardando</span><strong>{counts.waiting}</strong></button><button onClick={() => openLot(executions.find((item) => item.status === 'IN_PROGRESS')!)}><span>Em produção</span><strong>{counts.running}</strong></button><button data-state="attention" onClick={() => openLot(executions.find((item) => item.status === 'PAUSED')!)}><span>Parados</span><strong>{counts.paused}</strong></button><button onClick={() => openLot(executions.find((item) => item.status === 'COMPLETED')!)}><span>Concluídos</span><strong>{counts.completed}</strong></button><p><b>Evento ativo:</b> DC03 · Lote 266 · Ferramental · {eventDurationMinutes(events[0], currentTime)} min</p></section>
    <section className={styles.live} aria-labelledby="live-title"><header><div><h2 id="live-title">Live Production Timeline</h2><p>Scheduled × Actual · clique em um Lot ou Resource para acompanhar</p></div><span>Taxonomia demonstrativa</span></header>
      <div className={styles.timeline} data-testid="live-production-timeline"><div className={styles.axisCorner}>Resource</div><div className={styles.axis}>{[9,11,13,15,17,19,21].map((hour) => <span key={hour} style={{ left: `${((hour - 9) / 13) * 100}%` }}>{String(hour).padStart(2,'0')}:00</span>)}</div><div className={styles.timeLine} style={{ left: `calc(8rem + (100% - 8rem) * ${currentPosition / 100})` }}><span>{formatTime(currentTime)}</span></div>
      <div className={styles.lanes}>{FOUNDRY_RESOURCE_IDS.map((resourceId) => { const execution = executions.find((item) => item.resourceId === resourceId)!; const lot = definition.lots.find((item) => item.id === execution.lotId)!; const actualFinish = execution.actualFinish ?? (execution.status === 'NOT_STARTED' ? execution.scheduledStart : currentTime); const activeEvent = events.find((event) => event.resourceId === resourceId && event.status === 'ACTIVE'); return <section className={styles.lane} key={resourceId} data-state={execution.status}><button className={styles.resource} onClick={() => openResource(resourceId, execution)}><span className={styles.resourceHead}><strong>{resourceId}</strong><span>{stateLabel[execution.status]}</span></span><small>Lote {lot.lotNumber} · {execution.producedQuantity}/{execution.plannedQuantity}</small>{activeEvent ? <em className={styles.activeEventChip}><span aria-hidden="true">{eventIcon[activeEvent.eventType]}</span>{eventLabel[activeEvent.eventType]} · {eventDurationMinutes(activeEvent, currentTime)} min</em> : null}</button><div className={styles.tracks}><div><span>PLANEJADO</span><button className={styles.scheduled} style={{ left: `${timelinePosition(lot.scheduledStart, rangeStart, rangeFinish)}%`, width: `${timelineWidth(lot.scheduledStart, lot.scheduledFinish, rangeStart, rangeFinish)}%` }} onClick={() => openLot(execution)}>Lote {lot.lotNumber}</button></div><div><span>REAL</span>{execution.status !== 'NOT_STARTED' ? <button className={styles.actual} style={{ left: `${timelinePosition(execution.actualStart!, rangeStart, rangeFinish)}%`, width: `${timelineWidth(execution.actualStart!, actualFinish, rangeStart, rangeFinish)}%` }} onClick={() => openLot(execution)}>Lote {lot.lotNumber} · {execution.producedQuantity}/{execution.plannedQuantity}</button> : <span className={styles.notStarted} style={{ left: `${timelinePosition(lot.scheduledStart, rangeStart, rangeFinish)}%` }}>Ainda não iniciado</span>}{activeEvent ? <button className={styles.eventBlock} aria-label={`Evento ativo na ${resourceId}: ${eventLabel[activeEvent.eventType]}`} style={{ left: `${timelinePosition(activeEvent.startedAt, rangeStart, rangeFinish)}%`, width: `${timelineWidth(activeEvent.startedAt, currentTime, rangeStart, rangeFinish)}%` }} onClick={() => openLot(execution)}>Evento ativo</button> : null}</div></div></section>; })}</div></div>
    </section>
    <section className={styles.feed} aria-labelledby="feed-title"><header><div><h2 id="feed-title">Event Feed</h2><p>Mais recente primeiro · fatos operacionais demonstrativos</p></div><span>ACTIVE / CLOSED</span></header><div>{feed.map((event) => { const execution = executionsByLot[event.lotId]; const lot = definition.lots.find((item) => item.id === event.lotId)!; return <button key={event.eventId} data-status={event.status} onClick={() => setContext({ kind: 'LOT', lot, execution })}><time>{formatTime(event.startedAt)}</time><strong>{event.resourceId}</strong><span>Lote {lot.lotNumber}</span><b>{eventLabel[event.eventType]}</b><em>{event.status === 'ACTIVE' ? `ATIVO · ${eventDurationMinutes(event,currentTime)} min` : `${eventDurationMinutes(event,currentTime)} min · ENCERRADO`}</em></button>; })}</div></section>
    <details className={styles.disclosure}><summary>Como esses fatos preparam o OEE? <span>Ver detalhes</span></summary><div className={styles.oeeChain}><strong>Execution Facts + Production Events + Time</strong><span>→ Future Availability</span><strong>Planned + Produced + Time + Future Ideal Cycle</strong><span>→ Future Performance</span><strong>Produced + Future Quality Facts</strong><span>→ Future Quality → Future OEE</span></div></details>
    </div>{context ? <ContextDialog context={context} events={events} currentTime={currentTime} nextLot={contextNextLot} onClose={() => setContext(null)} /> : null}
