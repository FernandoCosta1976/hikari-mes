...
export type OperationalPerspective = 'PLAN' | 'READINESS' | 'EXECUTION' | 'MONITORING' | 'ADHERENCE' | 'QUALITY' | 'OEE';
...
function DemoJourneyBar({ pathname, search }: { pathname: string; search: string }) {
  ...
  return createPortal(<nav className={styles.journey} aria-label="Navegação guiada da demonstração"><small>Demo guiada · Etapa {journeyStep.id}/08</small><strong>{journeyStep.label}</strong><div>{previousStep ? <a href={previousStep.href}>← Anterior</a> : <span aria-hidden="true" />}{nextStep ? <a href={nextStep.href}>Próximo →</a> : <a href="/demo/fundicao-dc">Visão Executiva →</a>}</div></nav>, document.body);
}
...
export function OperationalWorkspace({ perspective, lotId, sidebarContent, children }: { perspective: OperationalPerspective; lotId?: string | null; sidebarContent: ReactNode; children: ReactNode }) {
  ...
  return <div className={styles.layout} data-sidebar-expanded={expanded}>
    ...
    <aside id="operational-workspace-sidebar" className={styles.sidebar} aria-label="Operational Workspace">
      <div className={styles.brand}><a href="/demo/fundicao-dc/production-scheduling" aria-label="HIKARI MES — Operational Workspace"><span className={styles.brandMark}>H</span><span className={styles.brandName}><b>HIKARI</b> MES</span></a><Button ref={toggleButtonRef} aria-label={expanded ? 'Recolher sidebar' : 'Expandir sidebar'} aria-expanded={expanded} aria-controls="operational-workspace-sidebar" onClick={() => setExpanded(!expanded)}>{expanded ? '«' : '»'}</Button></div>
      <nav className={styles.perspectives} aria-label="Perspectivas operacionais"><a href="/demo/fundicao-dc" title="Visão Executiva">⌂ <span>Visão Executiva<small>Programa HIKARI</small></span></a><strong title="Produção">P <span>Produção</span></strong><a aria-current={perspective === 'PLAN' ? 'page' : undefined} href={`/demo/fundicao-dc/production-scheduling${lotId ? `?lotId=${lotId}` : ''}`} title="Plano">▤ <span>Plano<small>O que precisamos produzir?</small></span></a><a aria-current={perspective === 'READINESS' ? 'page' : undefined} href="/demo/fundicao-dc/production-readiness" title="Preparação">✓ <span>Preparação<small>Temos condições de produzir?</small></span></a><a aria-current={perspective === 'EXECUTION' ? 'page' : undefined} href="/demo/fundicao-dc/production-execution" title="Execução">▶ <span>Execução<small>O que está sendo executado agora?</small></span></a><a aria-current={perspective === 'MONITORING' ? 'page' : undefined} href="/demo/fundicao-dc/production-monitoring" title="Acompanhamento">◉ <span>Acompanhamento<small>O que está acontecendo agora?</small></span></a><a aria-current={perspective === 'ADHERENCE' ? 'page' : undefined} href="/demo/fundicao-dc/production-adherence" title="Aderência">◈ <span>Aderência<small>Estamos conforme o planejado?</small></span></a><a aria-current={perspective === 'QUALITY' ? 'page' : undefined} href="/demo/fundicao-dc/production-quality" title="Qualidade & Performance">◆ <span>Qualidade &amp; Performance<small>Quanto produzimos e quanto perdemos?</small></span></a><a aria-current={perspective === 'OEE' ? 'page' : undefined} href="/demo/fundicao-dc/oee" title="OEE">● <span>OEE<small>Como estamos performando e por quê?</small></span></a></nav>
