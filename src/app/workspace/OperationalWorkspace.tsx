...
import { useScenarioPath } from '../routing/useScenarioPath';
import { useApplicationContext } from '../providers/ApplicationContext';
import { useWorkspaceSidebar } from '../providers/WorkspaceSidebarContext';
import { Button } from '../../shared/ui/Button/Button';
import { currentJourneyIndex, demoJourney } from './demoJourney';
...
function DemoJourneyBar({ pathname, search }: { pathname: string; search: string }) {
  ...
  const journey = demoJourney(scenarioId);
  const journeyIndex = currentJourneyIndex(pathname, search, scenarioId);
  if (journeyIndex === -1) return null;
  const journeyStep = journey[journeyIndex];
  const previousStep = journeyIndex > 0 ? journey[journeyIndex - 1] : undefined;
  const nextStep = journeyIndex < journey.length - 1 ? journey[journeyIndex + 1] : undefined;
  return createPortal(<nav className={styles.journey} aria-label="Navegação guiada da demonstração"><small>Demo guiada · Etapa {journeyStep.id}/08</small><strong>{journeyStep.label}</strong><div>{previousStep ? <a href={withBase(scenarioPath(previousStep.href))}>← Anterior</a> : <span aria-hidden="true" />}{nextStep ? <a href={withBase(scenarioPath(nextStep.href))}>Próximo →</a> : <a href={withBase(scenarioPath(''))}>Visão Executiva →</a>}</div></nav>, document.body);
}
...
export function OperationalWorkspace({ perspective, lotId, sidebarContent, children }: { perspective: OperationalPerspective; lotId?: string | null; sidebarContent: ReactNode; children: ReactNode }) {
  ...
  return <div className={styles.layout} data-sidebar-expanded={expanded}>
    ...
    <aside id="operational-workspace-sidebar" className={styles.sidebar} aria-label="Painel Operacional">
      <div className={styles.brand}><a href={withBase(scenarioPath('/production-scheduling'))} aria-label="HIKARI MES — Painel Operacional"><span className={styles.brandMark}>H</span><span className={styles.brandName}><b>HIKARI</b> MES</span></a><Button ref={toggleButtonRef} aria-label={expanded ? 'Recolher sidebar' : 'Expandir sidebar'} aria-expanded={expanded} aria-controls="operational-workspace-sidebar" onClick={() => setExpanded(!expanded)}>{expanded ? '«' : '»'}</Button></div>
      <nav className={styles.perspectives} aria-label="Perspectivas operacionais"><a href={withBase(scenarioPath(''))} title="Visão Executiva">⌂ <span>Visão Executiva<small>Programa HIKARI</small></span></a><a aria-current={perspective === 'STRATEGIC' ? 'page' : undefined} href={withBase(scenarioPath('/strategic'))} title="Visão Estratégica">★ <span>Visão Estratégica<small>Como está a saúde da Fundição DC?</small></span></a><strong title="Produção">P <span>Produção</span></strong><a aria-current={perspective === 'PLAN' ? 'page' : undefined} href={withBase(`${scenarioPath('/production-scheduling')}${lotId ? `?lotId=${lotId}` : ''}`)} title="Plano">▤ <span>Plano<small>O que precisamos produzir?</small></span></a><a aria-current={perspective === 'READINESS' ? 'page' : undefined} href={withBase(scenarioPath('/production-readiness'))} title="Preparação">✓ <span>Preparação<small>Temos condições de produzir?</small></span></a><a aria-current={perspective === 'EXECUTION' ? 'page' : undefined} href={withBase(scenarioPath('/production-execution'))} title="Execução">▶ <span>Execução<small>O que está sendo executado agora?</small></span></a><a aria-current={perspective === 'MONITORING' ? 'page' : undefined} href={withBase(scenarioPath('/production-monitoring'))} title="Acompanhamento">◉ <span>Acompanhamento<small>O que está acontecendo agora?</small></span></a><a aria-current={perspective === 'ADHERENCE' ? 'page' : undefined} href={withBase(scenarioPath('/production-adherence'))} title="Aderência">◈ <span>Aderência<small>Estamos conforme o planejado?</small></span></a><a aria-current={perspective === 'QUALITY' ? 'page' : undefined} href={withBase(scenarioPath('/production-quality'))} title="Qualidade & Desempenho">◆ <span>Qualidade &amp; Desempenho<small>Quanto produzimos e quanto perdemos?</small></span></a><a aria-current={perspective === 'OEE' ? 'page' : undefined} href={withBase(scenarioPath('/oee'))} title="OEE">● <span>OEE<small>Como estamos performando e por quê?</small></span></a></nav>
      <section className={styles.context} aria-label="Contexto da aplicação"><span aria-hidden="true">⌾</span><div><small>PowerTrain · Área Produtiva</small><label><span className={styles.srOnly}>Área Produtiva</span><select aria-label="Área Produtiva" value={productiveArea.id} onChange={(event) => setProductiveArea(event.target.value as typeof productiveArea.id)}>{availableProductiveAreas.map((area) => <option key={area.id} value={area.id}>{area.label}</option>)}</select></label><small>Cenário</small><strong>Demonstrativo</strong><span className={styles.srOnly}>Cenário demonstrativo</span></div></section>
